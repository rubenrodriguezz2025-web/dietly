/**
 * /plan/[planId] — Vista PWA del plan nutricional para el paciente.
 *
 * El middleware valida el token HMAC + expiración ANTES de llegar aquí.
 * Esta página solo necesita buscar el plan por su ID y renderizarlo.
 *
 * Seguridad:
 * - El middleware ya verificó: HMAC correcto, no expirado, planId coincide.
 * - Aquí se añade una segunda verificación: el plan debe estar en status 'approved'.
 * - Los headers Cache-Control: no-store los establece el middleware.
...
import { NavegadorDiasPlan } from './navegador-dias';
...
export const metadata: Metadata = {
  ...
  // Sin metadatos de Open Graph ni Twitter Card para evitar previews con datos clínicos
  robots: 'noindex, nofollow',
};

// ─── Página ──────────────────────────────────────────────────────────────────

export default async function PaginaPlanPaciente({
  params,
  searchParams,
}: {
  params: Promise<{ planId: string }>;
  searchParams: Promise<{ dia?: string; token?: string; expires?: string }>;
}) {
  const { planId } = await params;
  const { dia, token = '', expires = '' } = await searchParams;

  // Buscar el plan por su ID (el middleware ya validó el token HMAC)
  // Solo planes en estado 'approved' son accesibles
  ...
  // Ajustes de marca y datos profesionales del nutricionista
  let showMacros = true;
  ...
  if (pacienteData?.nutritionist_id) {
    const { data: perfil } = await (supabaseAdminClient as any)
      ...
      .single();
    if (perfil?.show_macros === false) showMacros = false;
    nombreDN = perfil?.full_name ?? null;
    colegiado = perfil?.college_number ?? null;
  }
  ...
  const fechaSemana = new Date(plan.week_start_date).toLocaleDateString(
    'es-ES',
    ...
    <>
      ...
      >
        ...
        <header
          ...
          style={{
            background:
              'linear-gradient(150deg, #0a3622 0%, #145c35 50%, #1a7a45 100%)',
        ...
        <main className='mx-auto max-w-lg px-4 pb-16'>
          <div className='anim-nav sticky top-0 z-10 -mx-4 border-b border-zinc-200/60 bg-[#f4f7f5]/95 px-4 pb-1 pt-3 backdrop-blur-md'>
            <NavegadorDiasPlan
        ...
        {/*
          Service Worker: registra sw.js para soporte offline del plan.
          Solo se registra en esta ruta (PWA del paciente).
          Sin Google Analytics, Meta Pixel ni tracking de terceros.
        */}
        <script
...
function TarjetaComida({
  ...
  const acento = ACENTO_TIPO[comida.meal_type] ?? {
    borde: '#16a34a',
    etiqueta: '#15803d',
  };
