...
export function LogoForm({
  ...
  const [uploadState, uploadAction, uploadPending] = useActionState(uploadLogo, initial);
  const [deleteState, deleteAction, deletePending] = useActionState(deleteLogo, initial);
    ...
    return (
      <div className='rounded-xl border border-dashed border-zinc-800 p-6 text-center'>
        <p className='text-sm text-zinc-500'>
          El logo de clínica en el PDF está disponible en el{' '}
    ...
    <div className='flex flex-col gap-5'>
      ...
      {(preview ?? currentLogoUrl) && (
        <div className='flex items-center gap-4'>
          <div className='relative flex h-20 w-40 items-center justify-center overflow-hidden rounded-lg border border-zinc-800 bg-white p-2'>
            <Image
          ...
          <p className='text-xs text-zinc-500'>
            {preview ? 'Vista previa del nuevo logo' : 'Logo actual'}
          </p>
        </div>
      )}
      ...
      <form action={uploadAction} className='flex flex-col gap-3'>
        ...
        {uploadState.success && (
          <p className='text-sm text-green-400'>Logo actualizado correctamente.</p>
        )}
        <button
          type='submit'
      ...
      {currentLogoUrl && !preview && (
        <form action={deleteAction}>
          ...
          >
            {deletePending ? 'Eliminando...' : 'Eliminar logo'}
          </button>
        </form>
      )}
    </div>
