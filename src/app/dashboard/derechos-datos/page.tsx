  ...
  return (
    <div className='flex flex-col gap-6'>
      ...
      <div className='flex items-start justify-between gap-4'>
        <div>
          <h1 className='text-xl font-bold text-zinc-900 dark:text-zinc-100'>Derechos RGPD</h1>
        ...
        <a
          ...
          className='flex-shrink-0 rounded-lg border border-zinc-300 px-3 py-1.5 text-xs text-zinc-600 transition-colors hover:border-zinc-400 hover:text-zinc-900 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-zinc-500 dark:hover:text-zinc-200'
        >
      ...
      <section className='flex flex-col gap-3'>
        ...
        {active.length === 0 ? (
          <div className='rounded-xl border border-zinc-200 bg-zinc-50 px-6 py-8 text-center dark:border-zinc-800 dark:bg-zinc-900/30'>
            <p className='text-sm text-zinc-600 dark:text-zinc-500'>No hay solicitudes pendientes.</p>
            <p className='mt-1 text-xs text-zinc-500 dark:text-zinc-600'>
              Cuando un paciente use{' '}
              <span className='text-zinc-700 dark:text-zinc-400'>/derechos-datos</span>, aparecerá aquí.
            </p>
          </div>
        ) : (
          <div className='flex flex-col gap-2'>
            {active.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className={`rounded-xl border bg-white px-4 py-4 dark:bg-zinc-900/40 ${
                    isOverdue
                ...
                >
                  <div className='flex flex-wrap items-start justify-between gap-3'>
                    <div className='flex flex-col gap-1.5'>
                      <div className='flex flex-wrap items-center gap-2'>
                        ...
                        {isOverdue ? (
                          ...
                          <span className={`text-xs ${days <= 5 ? 'text-amber-600 dark:text-amber-500' : 'text-zinc-500 dark:text-zinc-600'}`}>
                            {days} días restantes
                          </span>
                        )}
                      </div>
                      ...
                      {r.patient_email_snapshot && (
                        <p className='text-xs text-zinc-600 dark:text-zinc-500'>{r.patient_email_snapshot}</p>
                      )}
                      {r.notes && (
                        <p className='text-xs italic text-zinc-500 dark:text-zinc-600'>&ldquo;{r.notes}&rdquo;</p>
                      )}
                    ...
                    {r.patient_id && (
                      <Link
                        ...
                        className='flex-shrink-0 text-xs text-zinc-600 underline-offset-2 hover:text-zinc-900 hover:underline dark:text-zinc-600 dark:hover:text-zinc-400'
                      >
                        Ver ficha →
                      </Link>
                    )}
                  ...
                  <div className='mt-3 border-t border-zinc-200 pt-3 dark:border-zinc-800/60'>
        ...
        <section className='flex flex-col gap-3'>
          ...
          <div className='flex flex-col gap-2'>
            {resolved.map((r) => (
              <div
                key={r.id}
                className='flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 opacity-70 dark:border-zinc-800/60 dark:bg-zinc-900/20'
              >
                <div className='flex flex-wrap items-center gap-2'>
                  <RequestTypeBadge type={r.request_type} />
                  <StatusBadge status={r.status} />
      ...
      <div className='rounded-xl border border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800/40 dark:bg-zinc-900/20'>
        <div className='flex flex-wrap items-center justify-between gap-3'>
          <div>
            <p className='text-sm font-medium text-zinc-800 dark:text-zinc-300'>Plantilla de consentimiento informado</p>
            <p className='mt-0.5 text-xs text-zinc-600 dark:text-zinc-600'>
      ...
      {/* Info legal */}
      <div className='rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800/40 dark:bg-zinc-900/20'>
        <p className='text-xs leading-relaxed text-zinc-700 dark:text-zinc-400'>
          <strong className='text-zinc-900 dark:text-zinc-500'>Obligaciones legales:</strong> Debes responder en
