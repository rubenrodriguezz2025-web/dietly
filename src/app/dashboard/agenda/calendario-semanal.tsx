...
function CitaBloque({ cita }: { cita: Appointment }) {
  ...
  return (
    ...
    >
      ...
      <div className='pointer-events-none absolute left-full top-0 z-30 ml-2 hidden w-44 rounded-lg border border-zinc-200 bg-white px-3 py-2 shadow-xl group-hover:block dark:border-zinc-700 dark:bg-zinc-900'>
        <p className='font-medium text-zinc-900 dark:text-zinc-200'>{cita.time.substring(0, 5)} — {cita.patients?.name ?? 'Sin paciente'}</p>
        {cita.notes && (
          <p className='mt-1 text-[11px] leading-snug text-zinc-600 dark:text-zinc-500'>{cita.notes}</p>
        )}
        {cita.meeting_url && (
          <a
